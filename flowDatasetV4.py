...
class flowDataset(Dataset):
    ...
    def __init__(self,data_type='flow', val= False ,val_size = 0.2,transform=None):
        """
        Args:
        ...
        for class_path in file_list:
            for img_path in glob.glob(class_path+'/*.jpg'):
                ...
                label = df.loc[file_name,data_type]
        ...
        if(data_type == 'flow'):
            ...
            #Determine std and mean
            self.mean = statistics.mean(self.label)
            self.std = statistics.stdev(self.label)

            for i in self.label:
                
                self.std_label.append((i-self.mean)/self.std)
    ...
    def __getitem__(self, idx):
        ...
        if self.validation_set :
            img_path, label_name = self.val[idx]
        else:
